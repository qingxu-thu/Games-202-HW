...
Vec3f IntegrateBRDF(Vec3f V, float roughness, float NdotV) {
    ...
    Vec3f wi = sampleList[i].dir;
    float p = sampleList[i].pdf;
    float cost = wi.dot(N);
    float F1 = R_0+(1-R_0)*pow((1-cost),5);
    float F2 = R_1+(1-R_1)*pow((1-cost),5);
    float F3 = R_2+(1-R_2)*pow((1-cost),5);
    Vec3f H = (wi+V)/2;
    float D = DistributionGGX(N, H, roughness);
    float G = GeometrySmith(roughness,N.dot(V),N.dot(wi));
    A += F1*D*G/(p*N.dot(V)*N.dot(wi));
    B += F1*D*G/(p*N.dot(V)*N.dot(wi));
    C += F1*D*G/(p*N.dot(V)*N.dot(wi));
    }
