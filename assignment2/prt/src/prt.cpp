            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Eigen::Array3f Le(images[i][index + 0], images[i][index + 1],
                                      images[i][index + 2]);
                    double area = CalcArea(x,y,width,height);
					for (int l = 0; l <= SHOrder; ++l) {
						for (int m = -l; m <= l; ++m) {
							int coeffIndex = sh::GetIndex(l, m);
							auto coeff = Le * sh::EvalSH(l, m, dir.cast<double>().normalized())*area;
...
public:
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            int itr = 0;
            int sample_side = 2;
            Eigen::MatrixXf m_TransportSHCoeffsk;
            Eigen::MatrixXf m_TransportSHCoeffst;
            m_TransportSHCoeffsk.resize(SHCoeffLength, mesh->getVertexCount());
            m_TransportSHCoeffst.resize(SHCoeffLength, mesh->getVertexCount());
            while(itr<m_Bounce)
            {
            for (int i = 0; i < mesh->getVertexCount(); i++)
            {
            ...
            {if (itr==0)
                m_TransportSHCoeffsk.col(i).coeffRef(j) = m_TransportSHCoeffs.col(i).coeffRef(j);
            else
                {m_TransportSHCoeffsk.col(i).coeffRef(j) = m_TransportSHCoeffst.col(i).coeffRef(j);
                m_TransportSHCoeffs.col(i).coeffRef(j) += m_TransportSHCoeffst.col(i).coeffRef(j);
                }
            }
            }
            for (int i = 0; i < mesh->getVertexCount(); i++)
            ...
            const Normal3f &n = mesh->getVertexNormals().col(i);
              ...
              for (int t = 0; t < sample_side; t++) {
                for (int p = 0; p < sample_side; p++) {
                  ...
                  double H = n.dot(wi);
                  Ray3f r(v,wi);
                  if (H>0&&scene->rayIntersect(r)) {
                    Intersection it;
                    scene->rayIntersect(r,it);
                    for (int et=0;et<3;et++)
                    {
                        for (int l = 0; l <= SHCoeffLength; l++) 
                        {
                        m_TransportSHCoeffst.col(i).coeffRef(l) = m_TransportSHCoeffsk.col(it.tri_index[et]).coeffRef(l)*it.bary[et]*H;
                        }
                    }
                    }
                  else
                    {
                        for (int l = 0; l <= SHCoeffLength; l++) 
                            {
                            m_TransportSHCoeffst.col(i).coeffRef(l) = 0;
                            }
                    }
                }
              }
            ...
            for (int j = 0; j < SHCoeffLength; j++)
            {
                m_TransportSHCoeffst.col(i).coeffRef(j) *= weight;
            }
            }
            itr+=1;
        }
